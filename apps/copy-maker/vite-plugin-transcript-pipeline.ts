 ...
 * - GET  /api/pipeline-prompts-runtime (effective runtime prompt map)
 * - GET  /api/pipeline-prompts/reference?path=... (preview a referenced source file)
 * - PUT  /api/pipeline-prompts/:key (save prompt content)
 */
import type { IncomingMessage, ServerResponse } from 'node:http'
import type { Plugin } from 'vite'
// @ts-expect-error — Node ESM pipeline runner (no .d.ts)
...
import { listPipelinePrompts, savePipelinePrompt, getEffectivePipelinePrompts, readPipelinePromptReference, writePipelinePromptReference } from './server/transcript-pipeline/pipelinePrompts.mjs'

async function readMultipartText(
  req: IncomingMessage,
): Promise<{ text: string; filename: string; linkedinUrl?: string }> {
  const chunks: Buffer[] = []
  ...
  if (ct.includes('application/json')) {
    const json = JSON.parse(bodyStr) as { text?: string; filename?: string; linkedinUrl?: string }
    return {
      text: String(json.text || ''),
      filename: String(json.filename || 'transcript.txt'),
      linkedinUrl: typeof json.linkedinUrl === 'string' ? json.linkedinUrl : '',
    }
  }
...
async function handlePipelineRun(req: IncomingMessage, res: ServerResponse) {
  ...
  let payload: { text: string; filename: string; linkedinUrl?: string }
  try {
  ...
  try {
    for await (const ev of runTranscriptPipeline(payload.text, payload.filename, { linkedinUrl: payload.linkedinUrl })) {
...
async function handlePipelinePromptsRuntime(req: IncomingMessage, res: ServerResponse) {
  if (req.method !== 'GET') {
    res.statusCode = 405
    res.end('Method not allowed')
    return
...
function handlePipelinePromptReference(req: IncomingMessage, res: ServerResponse) {
  if (req.method === 'GET') {
    try {
      const rawUrl = req.url ?? ''
      const parsed = new URL(rawUrl, 'http://local')
      const referencePath = parsed.searchParams.get('path') || ''
      const data = readPipelinePromptReference(referencePath)
      res.statusCode = 200
      res.setHeader('Content-Type', 'application/json')
      res.end(JSON.stringify(data))
    } catch (e) {
  ...
  if (req.method === 'PUT') {
    void (async () => {
      try {
        ...
        const result = writePipelinePromptReference(referencePath, text)
        res.statusCode = 200
        res.setHeader('Content-Type', 'application/json')
...
export function transcriptPipelinePlugin(): Plugin {
  const handler = (
    ...
    if (pathname === '/api/pipeline-prompts-runtime') {
      void handlePipelinePromptsRuntime(req, res)
      return
    }
    if (pathname === '/api/pipeline-prompts/reference') {
      handlePipelinePromptReference(req, res)
      return
    }
