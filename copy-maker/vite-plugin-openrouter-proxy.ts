/**
 * Dev + `vite preview` (Railway): proxy `/api/openrouter/*` → `https://openrouter.ai/*` with the API key on the server.
 * OpenRouter does not allow browser cross-origin calls with API keys (no usable CORS for that flow), so the client
 * always POSTs to same-origin `/api/openrouter/v1/chat/completions` and this middleware forwards the stream.
 */
...
async function proxyOpenRouterToUpstream(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const apiKey =
    ...
    String(process.env.VITE_OPENROUTER_API_KEY ?? '').trim()
  ...
  if (!apiKey) {
    ...
    res.end(
      JSON.stringify({
        ...
        hint: 'Set OPENROUTER_API_KEY or VITE_OPENROUTER_API_KEY in the server environment (Railway Variables).',
      }),
    )
    return
  }
