...
function splitTurnsIntoBatches(turns, maxChars = 12_000) {
  const batches = []
  let current = []
  let currentChars = 0
...
export async function chunkTranscriptPass1(input, prompts = {}) {
  ...
  if (!promptOverride && !fs.existsSync(promptPath)) {
    throw new Error(`Chunking prompt not found: ${promptPath}`)
  }
...
/**
 * Reads active hypothesis context from environment variables.
 * The pipeline can also override this from database plan records.
 */
...
export async function interpretChunksPass2(input, prompts = {}) {
  if (!prompts.interpret_chunks_system_template && !fs.existsSync(PROMPT_HYPOTHESIS_INTERPRETATION)) {
    throw new Error(`Interpretation prompt not found: ${PROMPT_HYPOTHESIS_INTERPRETATION}`)
  }
